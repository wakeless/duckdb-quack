...
string QuackViewCatalogEntry::CreateViewSQL(const string &catalog_name, const string &schema_name,
	...
	//! View legs bind schema-only: pushdown routinely rewrites their query, which would discard
	//! an eagerly executed bind-time result and read the table for nothing.
